...
$(document).ready(function() {
    ...
    */
    ...
    function isExists(response, attribute) {
        ...
        for (var i = 0; i < data.length; i++) {
            // alert(data[i].name + " " + attribute);
            if (data[i].username == attribute) {
                return true;
            }
        }
        return false;
    ...
    function changePassword(accountUsername, accountNewPassword) {
        $.ajax({
            url: 'inc/account-management/data/change-password.php',
            data: {
    ...
    */
    ...
    $('#btn-change-password').on('click', function() {
        ...
        var accountPassword = $('#current-password').val();
        var accountNewPassword = $('#new-password').val();
        var accountConfirmNewPassword = $('#confirm-new-password').val();
        // Check exist data
        $.ajax({
            ...
            success: function(response) {
                if (accountNewPassword != accountConfirmNewPassword) {
                    toastr.warning('Mật khẩu nhập lại không trùng khớp!');
                } else if (accountPassword == accountNewPassword) {
                    toastr.warning('Mật khẩu mới không được trùng mật khẩu cũ!');
                } else if (!isExists(response, accountUsername)) {
                    toastr.warning('Tài khoản này không tồn tại!');
                } else {
                    $.ajax({
                        url: 'inc/account-management/data/correct-password.php',
                        data: {
                            accountUsername: accountUsername,
                        ...
                        success: function(response) {
                            if (response.indexOf("KHÔNG") != -1) {
                                toastr.error('Mật khẩu sai!');
                            } else {
                                changePassword(accountUsername, accountNewPassword);
                            }
                        },
                        error: function() {
                            toastr.error('Mật khẩu sai!');
