...
$(document).ready(function() {
    ...
    */
    ...
    function getAccountInformation(accountUsername) {
        $.ajax({
            url: 'inc/account-information/data/get-account-information.php',
            data: { accountUsername: accountUsername },
            type: 'POST',
            success: function(response) {
                //alert(response);
                if (response.indexOf("KHÔNG") != -1) {
                    toastr.error('Không thể lấy thông tin tài khoản!');
                } else {
                    response = JSON.parse(response);
                    $('#username').val(response.username);
                    $('#display-name').val(response.displayName);
                    $('#type-name').val(response.accountTypeName);
    ...
    function setEditable(allowEditable) {
        $('#display-name').prop('readonly', !allowEditable);
        $('#type').prop('readonly', !allowEditable);
        $('#gender').prop('disabled', !allowEditable);
        $('#birthday').prop('readonly', !allowEditable);
        $('#address').prop('readonly', !allowEditable);
    }
    ...
    */
    ...
    var accountUsername = $('#current-username').val();
    getAccountInformation(accountUsername);

    // Change account information
    $('#btn-change-information').on('click', function() {

        allowEditable = !allowEditable;
        setEditable(allowEditable);

        if (!allowEditable) {
            // Get account info
            var accountUsername = $('#username').val();
            var accountDisplayName = $('#display-name').val();
            var accountTypeName = $('#type-name').val();
            var accountGender = $('#gender').val();
            var accountBirthday = $('#birthday').val();
            var accountAddress = $('#address').val();

            $.ajax({
                url: 'inc/account-information/data/update-account-information.php',
                data: {
                ...
                success: function(response) {
                    if (response.indexOf("KHÔNG") != -1) {
                        toastr.error('Thay đổi thông tin tài khoản không thành công!');
                    } else {
                        toastr.success('Thay đổi thông tin tài khoản thành công!');
                        $('#current-display-name').text(accountTypeName + ' - ' + accountDisplayName);
                    }
                },
                error: function() {
                    toastr.error('Thay đổi thông tin tài khoản không thành công!');
